...
int main(int argc, char** argv){
  ...
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
  {
    ...
    //sleep(5000);
     ...
     ros::param::set("/robot_status", 1); // 1 means object at pickup point 
  ...
  // Define a position and orientation for the robot to reach
    drop_zone.target_pose.pose.position.y = -7.0;
    drop_zone.target_pose.pose.position.x = 4.7;
  ...
  // Check if the robot reached its goal
    if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
    {
         ...
         ros::param::set("/robot_status", 2); 
    }     
    else
    {
        ROS_INFO("couldn't reach the dropoff zone");
    }
  ...
  { ROS_INFO("couldn't reach the pickup zone");}
